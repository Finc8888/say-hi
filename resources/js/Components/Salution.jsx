...
export default function Salution({ salution }) {
    ...
    const { data, setData, patch, clearErrors, reset, errors } = useForm({
        message: salution.message,
    });

    const submit = (e) => {
        e.preventDefault();
        patch(route('salutions.update', salution.id), { onSuccess: () => setEditing(false) });
    };
            ...
            <div className="flex-1">
                <div className="flex justify-between items-center">
                    <div>
                        ...
                        { salution.created_at !== salution.update_at &&
                            <small className="text-sm text-gray-600"> &middot; edited</small>
                        }
                    </div>
                    {
                        salution.user.id === auth.user.id &&
                        <Dropdown>
                            <Dropdown.Trigger>
                                <button>
                                    <svg
